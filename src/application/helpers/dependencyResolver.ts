...
export class DependencyResolver {
    ...
    register({ name, model, services }: { name: string; model: unknown; services: string[] }): void {
    ...
    get<M = unknown, T = unknown>(name: string, ...args: T[]): M {
        ...
        return new Model(...services.map((servicesName): unknown => this.services[servicesName]), ...args) as M;
    }
}
